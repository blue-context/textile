...
async def test_acompletion_without_transformers(sample_messages, mock_completion_response, stream):
    ...
    with patch("litellm.acompletion", new_callable=AsyncMock, return_value=mock_completion_response):
...
async def test_acompletion_with_transformers(sample_messages, mock_completion_response, mock_transformer):
    ...
    with patch("litellm.acompletion", new_callable=AsyncMock, return_value=mock_completion_response):
        with patch("textile.lite.completion.get_config") as mock_config:
            ...
            with patch("litellm.get_max_tokens", return_value=4096):
                ...
                result = await acompletion(model="gpt-4", messages=sample_messages, transformers=[mock_transformer])
                assert result is not None
                mock_transformer.transform.assert_called_once()


@pytest.mark.parametrize("max_tokens", [None, 2048, 8192])
async def test_acompletion_max_tokens_handling(sample_messages, mock_completion_response, max_tokens):
...
async def test_acompletion_with_tools(sample_messages, mock_completion_response):
    ...
    with patch("litellm.acompletion", new_callable=AsyncMock, return_value=mock_completion_response) as mock_llm:
        with patch("textile.lite.completion.get_config") as mock_config:
            mock_config.return_value.transformers = None
            await acompletion(model="gpt-4", messages=sample_messages, tools=tools, tool_choice="auto")
    ...
    with patch("litellm.acompletion", new_callable=AsyncMock, return_value=mock_completion_response):
        with patch("textile.lite.completion.get_config") as mock_config:
            ...
            with patch("litellm.get_max_tokens", return_value=4096):
                ...
                result = await acompletion(model="gpt-4", messages=sample_messages, transformers=[mock_transformer], debug=True)
