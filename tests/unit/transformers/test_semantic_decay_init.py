...
class TestMessageType:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (MessageType.INSTRUCTION, 0.9),
            (MessageType.FACTUAL, 0.8),
            (MessageType.CONVERSATIONAL, 0.6),
...
class TestSemanticDecayTransformerInit:
    ...
    def test_validates_semantic_threshold(self, semantic_threshold, valid):
        if valid:
            assert (
                SemanticDecayTransformer(semantic_threshold=semantic_threshold).semantic_threshold
                == semantic_threshold
            )
        else:
    ...
    @pytest.mark.parametrize(
        "semantic_weight,temporal_weight,valid",
        [
            (0.6, 0.4, True),
            (0.5, 0.5, True),
            (1.0, 0.0, True),
